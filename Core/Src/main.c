...
#define MARIO_LEN (sizeof(mario_notes)/sizeof(uint16_t))
void MarioMusicTask(void *argument)
{
    ...
    for (;;)
    {
    	uint16_t note = mario_notes[i];
    	        uint16_t tempo = mario_duration[i];

    	        uint32_t noteTime = 800 / tempo;   // CHUẨN MARIO
    	        ...
    	        {
    	            Buzzer_SetFrequency(note);
    	        }

    	        osDelay(noteTime);

    	        // Pause giữa các nốt (30%)
    	        Buzzer_Stop();
    	        osDelay(noteTime * 0.3f);
...
osThreadAttr_t marioTask_attributes = {
  .name = "MarioMusic",
  .stack_size = 512 * 4,
