...
\subsubsection{MainViewPresenter}
...
\begin{itemize}
    \item **Navigation:** Handles the transition event when the user selects "Start Game", requesting the application to switch to the Game Screen.
    \item **Data Retrieval:** Fetches stored high scores from the Model (`getHighScores`) to display the leaderboard on the menu, enticing the player to beat the top records.
\end{itemize}

\subsubsection{GameViewPresenter}
...
\begin{itemize}
    \item **Input Proxying:** It receives user input events from the View (e.g., button presses for left, right, rotate) and forwards them to the Model via methods like `handleLeft()` or `handleRotate()`.
    \item **State Synchronization:** It implements the `modelStateChanged()` method. When the game tick updates the Model (gravity, collisions), this method is called to trigger `view.updateBoard()`, ensuring the screen redraws at 60 FPS.
    \item **Scoreboard Logic:** It implements complex logic for the Game Over screen. The `getScoreboard()` method retrieves the top 3 high scores, mixes in the current player's score, and sorts them dynamically. This allows the player to see their rank immediately upon finishing a game (`ScoreInfo` structure).
\end{itemize}

\begin{lstlisting}[language=C++]
// Example: Synchronization in GameViewPresenter.cpp
