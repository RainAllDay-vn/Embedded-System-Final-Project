...
\begin{document}
	...
	\subsection{The Presenter: Logic-UI Sychnorization}
	...
	The Model class encapsulates the entire state of the Tetris game. It operates independently of the View and Presenter, ensuring that the game rules (gravity, collisions, scoring) are consistent regardless of how they are displayed.
	
	\subsubsection{Grid Representation}
	The game field is represented by a 2D array, `grid[20][10]`. Each cell stores a value representing the type of Tetromino occupying it, or `-1` if the cell is empty. This simple data structure allows for efficient collision detection and line-clearing operations.
	
	\subsubsection{The Game Loop (Tick)}
	The `tick()` function is the heartbeat of the game. It is called periodically (based on the game speed) to advance the game state. Its primary responsibilities are:
	\begin{itemize}
		\item Gravity: Moving the current piece down one row.
		\item Input Processing: Consuming user commands (Left, Right, Rotate, Drop) from the `inputQueue`.
		\item Collision Checks: verifying if the piece has landed or hit a wall.
	\end{itemize}
	
	\begin{lstlisting}[language=C++, caption={Game Loop Logic in Model::tick}]
		...
		void Model::tick()
		{
			...
			{
				tickCounter = 0;
				step(); // Try to move piece down
			}
			
			// 2. Process Input Queue from FreeRTOS
			#ifndef SIMULATOR
			uint8_t key = 0;
